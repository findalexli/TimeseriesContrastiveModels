...
#run = wandb.init(name=experiment_description + run_description, config=configs)
...
TFC_model = TFC_resnet(configs).to(device)
...
if training_mode == "fine_tune_test":
    ...
    load_from = '/home/ubuntu/TFC-pretraining/code/experiments_logs/SleepEEG_2_Epilepsy/run1/pre_train_seed_42_2layertransformer/saved_models/'
    print("The loading file path", load_from)
    chkpoint = torch.load(os.path.join(load_from, "ckp_last.pt"), map_location=device)
    pretrained_dict = chkpoint["model_state_dict"]
    TFC_model.load_state_dict(pretrained_dict)
    pytorch_total_params = sum(p.numel() for p in TFC_model.parameters())
    print("Total number of parameters in TFC model: ", pytorch_total_params)
