...
def load_baseline(dataset, transpose= True):
    if transpose:
        train_input = np.transpose(np.load(os.path.join('datasets', dataset, 'train_input.npy')), axes=(0,2,1))
         # window_len and num_channels dim are exchanged
        test_input = np.transpose(np.load(os.path.join('datasets', dataset, 'test_input.npy')), axes=(0,2,1))
    else:
        train_input = np.load(os.path.join('datasets', dataset, 'train_input.npy'))
        test_input = np.load(os.path.join('datasets', dataset, 'test_input.npy'))

    if os.path.exists(os.path.join('datasets', dataset, 'train_output.npy')) and \
        os.path.exists(os.path.join('datasets', dataset, 'test_output.npy')):
    ...
    train_output = np.random.randint(0, 2, train_input.shape[0])
    test_output = np.random.randint(0, 2, test_input.shape[0])
    return train_input, train_output, test_input, test_output
