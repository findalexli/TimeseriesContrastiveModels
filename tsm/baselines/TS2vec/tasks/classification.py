...
def eval_classification(model, train_data, train_labels, test_data, test_labels, eval_protocol='linear'):
    ...
    train_repr = model.encode(train_data, encoding_window='multiscale' if train_labels.ndim == 1 else None)
    test_repr = model.encode(test_data, encoding_window='multiscale' if train_labels.ndim == 1 else None)
...
#     print(train_labels)
    #test_labels_onehot = label_binarize(test_labels, classes=np.arange(train_labels.max()+1)) # Uncommendted by Alex Li 
...
#     print(test_labels_onehot)
    auprc = average_precision_score(test_labels, y_score[:, 0])
    return y_score, {'AUPRC': auprc}
#    print(y_score)
#    print(test_labels)
    metrics_dict = {}
    pred_prob = y_score
